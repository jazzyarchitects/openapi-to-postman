...
class SchemaPackOrchestrator {
  constructor(input, options = {}) {
    if (global.isValidationWorker) {
      throw new Error('Orchestrator invoked in validation worker');
    }
    ...
    this.input = input;
    this.options = _.assign({ max_workers: 2, timeout: 5 * 1000, lockRetries: 5 * 1000 / 20 }, options);
  }

  validate (transactions, startTime, callback) {
    if (!callback && typeof startTime === 'function') {
      callback = startTime;
      startTime = Date.now();
    }
    const worker = fork(path.join(__dirname, 'worker', 'validate-transaction.js'),
        ['validate', '--input', Buffer.from(JSON.stringify(this.input), 'utf-8').toString('base64'),
          '--transactions', Buffer.from(JSON.stringify(transactions), 'utf-8').toString('base64')]),
      interval = setInterval(() => {
        if (Date.now() - startTime > this.options.timeout) {
          clearInterval(interval);
          worker.kill();
          validationWorkerCount -= 1;
          return callback(new Error('ValidationTimeoutError'));
        }
      }, 100);

    global.validationWorkerCount += 1;

    worker.on('message', function (message) {
      clearInterval(interval);
      validationWorkerCount -= 1;

      return callback(null, JSON.parse(Buffer.from(message, 'base64').toString()));
    });
  ...
  validateTransaction (transaction, callback) {
    if (global.validationWorkerCount < this.options.max_workers) {
      return this.validate(transaction, callback);
    }
    ...
    const startTime = Date.now(),
      interval = setInterval(() => {
        if (Date.now() - startTime > this.options.timeout || retryCount > this.options.lockRetries) {
          clearInterval(interval);

          return callback(new Error('MaxConcurrencyLockTimeoutError'));
        }
        ...
        if (global.validationWorkerCount < this.options.max_workers) {
          ...
          return this.validate(transaction, startTime, callback);
        }
